...
$('logoutBtn').addEventListener('click',async()=>{await signOut(auth);location.href='login.html'});$('refreshBtn').addEventListener('click',renderAll);$('lotterySelect').addEventListener('change',async e=>{lot=lots.find(x=>x.id===e.target.value)||lot;await renderAll()});$('editBtn').addEventListener('click',editLottery);
onAuthStateChanged(auth,async u=>{if(!u){location.href='login.html';return}me=u;try{const p=await getDoc(doc(db,'users',u.uid));if(!p.exists()||p.data().role!=='lotteryAdmin'){location.href='login.html';return}profile=p.data();const ids=profile.lotteryIds||[];if(!ids.length)throw new Error('No lottery assigned to this account.');const snaps=await Promise.all(ids.map(id=>getDoc(doc(db,'lotteries',id))));lots=snaps.filter(s=>s.exists()).map(s=>({id:s.id,...s.data()}));if(!lots.length)throw new Error('None of your assigned lotteries are available.');$('lotterySelect').innerHTML=lots.map(x=>`<option value="${esc(x.id)}">${esc(x.name)}</option>`).join('');lot=lots[0];await renderAll()}catch(e){console.error(e);$('message').innerHTML=`<div class="error">${esc(e.message)}</div>`}});
