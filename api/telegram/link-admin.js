...
const { adminDb, FieldValue, adminBotLink } = require('./_lib');
...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    return res.status(200).json({ok:true,url:adminBotLink(`admin_${token}`),token,expiresInSeconds:600,botUsername:process.env.TELEGRAM_ADMIN_BOT_USERNAME||''});
